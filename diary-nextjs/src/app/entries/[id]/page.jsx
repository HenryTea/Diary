...
export default function Page() {
  ...
  useEffect(() => {
    if (isNewEntry) {
      // For new entries, create a blank entry object
      setEntry({
        id: 'new',
        text: '',
  ...
  const handleDelete = async () => {
    if (isNewEntry) return; // Don't allow deleting new entries
      ...
      <div className="sticky top-0 z-40 w-full bg-[#e9f3f6] flex flex-col items-center border-b border-gray-200">
        <div className="w-full flex justify-between items-center px-4 py-2 relative h-12 overflow-hidden">
          <SaveButton 
            ...
            onBack={() => router.push("/")}
            saveText={isNewEntry ? "Create" : "Save"}
          />
          
          <span className="text-xs font-semibold tracking-widest ml-50">
            {isNewEntry ? "New Entry" : new Date(entry.date).toLocaleString("en-US")}
          </span>
